...
from DustShape import Cube, SphericalBlub
...
class SurfaceBrightness:
    ...
    # def calculate_surface_brightness(self, DustShape):
    #     """
    ...
    #             S[ik] = 0
    #         if (isinstance(DustShape, Cube) or isinstance(DustShape, SphericalBlub)):
    #             Inte_z = np.arcsinh(self.z_inter_values[ik]/rhos[ik])
    #             temporal_s =  Ir * S[ik] * Inte_z / ( 4 * np.pi )
    #             self.surface[ik] = temporal_s
    #         else:
    #             Inte_z = self.dz0
    #             self.surface[ik] = Ir * S[ik] * Inte_z / ( 4 * np.pi * r[ik] * rhodrho[ik] )

    #     return self.cossigma, self.surface
    ...
    def calculate_surface_brightness_1(self, DustShape):
        """
            Calculate the surface brightness at a position r = (x_inter, y_inter, z_inter): 
            Sugermann 2003 equation 7:
                SB(lambda, t) = F(lambda)nH(r) * (c dz0 / (4 pi r rhodrho) )* S(lambda, mu) 
    ...
    """
        ...
        for ik, rm in enumerate(self.cossigma):
            ...
            if isinstance(DustShape, Cube):
                ...
                def objective_function(surface_val):
                    integral_value = integrate.simpson(surface_val * rhos, rhos)  
                    return abs(integral_value - temporal_s) 
                # Initial guess for surface brightness
                initial_guess = 1
                # Minimize the objective function to find surface brightness
                result = optimize.minimize(objective_function, initial_guess)
                # self.surface[ik] = Ir * S[ik] * Inte_z / ( 4 * np.pi * rhodrho[ik] )
                self.surface[ik] = result.x
            else:
                Inte_z = self.dz0
                self.surface[ik] = Ir * S[ik] * Inte_z / ( 4 * np.pi * r[ik] * rhodrho[ik] )
                ...
                for i in range(size_x):
                    for j in range(size_y):
                        ...
                        if arr.shape[0] == 0:
                            ...
                            if ((mins == maxs) & (mins != 0)):
                                Inte_z_min = np.arcsinh((mins-0.00001)/rhos(mins-0.00001))
                            temporal_s =  Ir * S[j,i] * (Inte_z_max-Inte_z_min) / ( 4 * np.pi )
