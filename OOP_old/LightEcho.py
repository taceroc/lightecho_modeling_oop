...
class LE_plane(LE):
    ...
    #     """
    #     r_le = np.sqrt(self.r_le2)
    #     xmin = -r_le - (self.A/self.F)*self.ct - (self.B/self.F)*self.ct
    #     xmax = r_le - (self.A/self.F)*self.ct - (self.B/self.F)*self.ct
...
class LE_Cube(LE):
    def __init__(self, ct, cube, source):
        """
            x: initialize x positions in ly
            ct: time of LE observation in years
            sphere.eq_params = [A, B, C, D], (x)2 + y2 + z2 = D2
        ...
        """
        super().__init__(ct, source)
        self.A = cube.eq_params[0]
        self.B = cube.eq_params[1]
        self.F = cube.eq_params[2]
        self.D = cube.eq_params[3]
        self.D = -min(cube.z2, cube.z1)
        self.check_time_forLE()
        self.dz0 = cube.dz0
        self.z_depths = cube.calculate_step_planes_cube()
        self.xy_matrix = []
        self.z_matrix = []
        self.pixel_xs_true = []
        self.pixel_ys_true = []
    ...
    def calculate_rle2(self):
        ...
        self.r_le2 = -2 * self.ct * (self.z_depths/self.F) + (self.ct)**2 * (1 + (self.B/self.F)**2 + (self.A/self.F)**2)
        return self.r_le2
    ...
    def get_intersection_xyz(self, cube):
        ...
        for iy in range(cube.size_z):
            x_inter_value = np.sqrt(self.r_le2[iy]) * np.cos(theta_p) - (self.A/self.F)*self.ct
            y_inter_value = np.sqrt(self.r_le2[iy]) * np.sin(theta_p) - (self.B/self.F)*self.ct
            x_p, y_p = np.meshgrid(x_inter_value, y_inter_value)
            # calculate the z intersections for each "infinitesimal" plane
            z_inter_value = -(self.z_depths[iy]/self.F) - (self.A * x_p / self.F) - (self.B * y_p / self.F)
            intersection_points = utils.cal_inter_cube(x_p, y_p, z_inter_value, cube.x_min, cube.x_max, cube.y_min, cube.y_max, cube.z_min, cube.z_max, self.ct, self.dt0)
            self.x_inter_values.append([inter[0] for inter in intersection_points])
            self.y_inter_values.append([inter[1] for inter in intersection_points])
            self.z_inter_values.append([inter[2] for inter in intersection_points])
            
        self.x_inter_p_new = np.concatenate(self.x_inter_values)
        self.y_inter_p_new = np.concatenate(self.y_inter_values)
        self.z_inter_p_new = np.concatenate(self.z_inter_values)
    ...
    def XYZ_merge_plane_cube_2ddust(self, cube, dust_cube_img, x_inter_p_new, y_inter_p_new, z_inter_p_new):
        """
        ...
        pixel_z = ((z_inter_p_new - cube.z_min) / (cube.z_max -  cube.z_min)) * (cube.size_z - 1)

        self.pixel_xs_true = np.round(pixel_x).astype(int) + 1
        self.pixel_ys_true = np.round(pixel_y).astype(int) + 1
        self.pixel_zs_true = np.round(pixel_z).astype(int) + 1
        ...
        self.xy_matrix = np.zeros((cube.side_y, cube.side_x, 2))
        self.z_matrix = np.zeros((cube.side_y, cube.side_x, cube.size_z))
        ...
        z_bins = np.linspace(np.nanmin(z_inter_p_new), np.nanmax(z_inter_p_new), cube.size_z)
        ...
        for i, up in enumerate(unique_pairs):
            ...
            val = np.unique(zi, return_counts=True)[0]
            for j in val:#range(1, 45):
                self.z_matrix[y_ind, x_ind, j-1] = z_bins[j-1] * dust_cube_img[y_ind, x_ind, j-1]#* dust_nobool[y_ind, x_ind, j-1]
