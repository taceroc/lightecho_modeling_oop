...
from DustShape import Cube, InfPlane, SphereCenter, SphericalBlub, PlaneDust
from LightEcho import LE_Cube, LE_plane, LE_sphere_centered, LE_SphericalBulb, LE_PlaneDust
...
def cube(dz0, ct, dt0, dust_position, size_cube, params, source1, save = False):
    img = gcd.generate_cube_dust() #generate_cube_dust_random()
    # dust_nobool = gcd.generate_cube_dust_nonbool()
    dust_cube_img = img.copy()

    dust_shape = np.array(dust_cube_img.shape)
    cube1 = Cube(params, dz0, dust_shape, dust_position, size_cube )
    ...
    if start_t == ct:
        start_t = ct - dt0
    for obs_time in np.arange(start_t, ct, 1 * fc.dtoy):
        print(f"Time {obs_time / fc.dtoy} day")
        ct = obs_time
        # try:
        LE_cube1source1 = LE_Cube(ct, cube1, source1)
        (x_inter_p_new, y_inter_p_new, z_inter_p_new) = LE_cube1source1.get_intersection_xyz(cube1)

        # new_xs = utils.convert_ly_to_arcsec(source1.d, x_inter_p_new)
        # new_ys = utils.convert_ly_to_arcsec(source1.d, y_inter_p_new)
        # new_zs = utils.convert_ly_to_arcsec(source1.d, z_inter_p_new)
        
        x_inter.extend(x_inter_p_new)
        y_inter.extend(y_inter_p_new)
        z_inter.extend(z_inter_p_new)
        # except:
        #     continue


    x_total = np.array(x_inter)
    y_total = np.array(y_inter)
    z_total = np.array(z_inter)
    LE_cube1source1 = LE_Cube(ct, cube1, source1)
    (pixel_xs_true, pixel_ys_true, pixel_zs_true, xy_matrix, z_matrix, 
     x_bins, u_bins, z_bins) = LE_cube1source1.XYZ_merge_plane_cube_2ddust(cube1, dust_cube_img, x_total, y_total, z_total)

    print(z_matrix.shape)
    flat = xy_matrix.reshape(cube1.side_x*cube1.side_y,2)
    cord = np.array([(xf, yf) for xf, yf in flat if xf != 0 or yf != 0])
    arris = []
    for i in range(cube1.side_x):
        for j in range(cube1.side_y):
            for k in range(cube1.size_z):
                arris.append([xy_matrix[j, i, 0], xy_matrix[j, i, 1], z_matrix[j, i, k]])

    arris = np.array(arris)
    cossigma, surface_total = SurfaceBrightness(source1, 
                                    LE_cube1source1, 
                                    [arris[:, 0], arris[:,1], arris[:,2]]).calculate_surface_brightness_2(cube1, z_matrix, xy_matrix, cube1.side_x, cube1.side_y)
    ...
    surface_val, surface_img, x_img, y_img = LE_image_nonanalytical(x_inter_arcsec_total, y_inter_arcsec_total, surface_total, pixel_resolution = 0.2, cmap = 'magma_r').create_le_surface(cube1)


    n_speci = f"{CUBE_NAME}_dt0_{int(dt0 / fc.dtoy)}_ct{int(ct / fc.dtoy)}_c{dust_position}_size{size_cube}{dust_shape}dz0{dz0}"
    info_le = [{
    ...
    figs1.add_trace(go.Scatter3d(x = [cube1.x1, cube1.x2, cube1.x2, cube1.x1, cube1.x1, cube1.x2, cube1.x2, cube1.x1],
                      y = [cube1.y1, cube1.y1, cube1.y2, cube1.y2, cube1.y2, cube1.y2, cube1.y1, cube1.y1,],
                      z = [cube1.z1, cube1.z1, cube1.z1, cube1.z1, cube1.z2, cube1.z2, cube1.z2, cube1.z2], showlegend = False))
...
# ct =  180 * fc.dtoy #180
# dt0 = 180 * fc.dtoy #180
# dust_position = np.array([2, 1, 13])#np.array([-50, 120, 25e3])#ly * fc.pctoly
# size_cube = np.array([1,1]) #fc.pctoly #ly
# params = [0,0,1,0]
# source1 = Source(dt0, vc.d, dc.Flmax)
# cube(dz0, ct, dt0, dust_position, size_cube, params, source1, save=True)
...
# xp = np.linspace(-10,10,100)
# xp, yp = np.meshgrid(xp, xp)
# zp = (xp**2 + yp**2 - (ct**2))/(2*ct)
# intersection_points = utils.cal_inter(xp, yp, zp, A, B, F, ct, D, dt0)

# x_inter_p_new = np.array([inter[0] for inter in intersection_points])
# y_inter_p_new = np.array([inter[1] for inter in intersection_points])
# z_inter_p_new = np.array([inter[2] for inter in intersection_points])
...
#             )
# figs.add_trace(go.Surface(
#     x=A + D * np.cos(phi) * np.sin(theta),
#     y=B + D * np.sin(phi) * np.sin(theta),
#     z=F + D * np.cos(theta),
#     opacity=1,
#     colorscale='YlGnBu')
#             )
