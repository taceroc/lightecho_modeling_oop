...
class Cube(DustShape):
    ...
    def __init__(self, eq_params, dz0, dust_shape, dust_position, size_cube):
        ...
        self.z1 = dust_position[2] + self.dz0/2
        self.z2 = dust_position[2] - self.dz0/2
        self.x_min = min(self.x1, self.x2)
        self.x_max = max(self.x1, self.x2)
        self.y_min = min(self.y1, self.y2)
        self.y_max = max(self.y1, self.y2)
        self.z_min = min(self.z1, self.z2)
        self.z_max = max(self.z1, self.z2)

            
    def calculate_step_planes_cube(self):
        F = self.eq_params[2]
        step = abs(self.z1 - self.z2) / (self.size_z-1)
        increases = np.array([i*step for i in range(self.size_z)])
        self.z_depths = -min(self.z1, self.z2)*F - F*increases
        return self.z_depths
